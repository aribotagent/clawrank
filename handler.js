...
const API_URL = process.env.LEADERBOARD_URL || "https://clawrank-production.up.railway.app";
const HOME = process.env.HOME || "";
const LOG_FILE = join(HOME, ".openclaw", "labor-hook.log");
const CONFIG_FILE = join(HOME, ".openclaw", "workspace", "skills", "labor-leaderboard", "config.json");
const STATE_FILE = join(HOME, ".openclaw", "labor-hook-state.json");
const AGENTS_DIR = join(HOME, ".openclaw", "agents");
const REPORT_INTERVAL_MS = 3 * 60 * 60 * 1000; // 3 小时
...
function loadConfig() {
  try {
    ...
    return {
      ...
      country: parsed.country ? String(parsed.country).toUpperCase() : "SG",
...
function registerFallback() {
  ...
  try {
    ensureDir(join(HOME, ".openclaw", "workspace", "skills", "labor-leaderboard"));
...
function saveState() {
  try {
    ...
    log(`Failed saving state: ${err.message}`);
  }
}
...
function accumulateDeltas() {
  ...
  for (const filePath of files) {
    ...
    for (const [model, totals] of Object.entries(currentByModel)) {
      const prev = (previous.models && previous.models[model]) || { tokens: 0, input: 0, output: 0 };
      addDelta(model, {
...
function postReport(body) {
  return new Promise((resolve) => {
    const url = new URL(API_URL + "/api/report");
    ...
    request.on("error", (err) => {
      log(`Report failed model=${body.model}: ${err.message}`);
      resolve(false);
    });
...
async function report(config) {
  ...
  for (const [model, delta] of modelDeltas.entries()) {
    ...
    const ok = await postReport({
      agent_id: config.gateway_id,
      agent_name: config.agent_name,
      message: config.message || "",
      country: config.country || "SG",
...
module.exports = async function handler(event) {
  ...
  if (event && event.type === "command" && ["new", "reset", "report"].includes(event.action)) {
    accumulateDeltas();
    await report(config);
    return;
  }
